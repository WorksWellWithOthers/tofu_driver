...
| Shop | Idle-first progression principle | Documented only | `DESIGN.md`, `BALANCE_AND_PROGRESSION.md`, `EXTERNAL_REFERENCE_INCREMENTAL_GAME_DESIGN_TRANSCRIPTS_AUDIT.md` | None | Validate through playtesting and automation-exit tests as repeated actions are added | P0 | Manual actions teach the loop, automation replaces repeated labor, decisions rather than clicking speed drive progression, and every repeated manual action needs an automation exit |
...
| Shop | Next Milestone Bar V1 | Implemented | `nextMilestoneForShop`, `renderNextMilestoneCard`, Overview milestone card CSS in `frontend/nospill/app.js` and `frontend/nospill/app.css` | `testTofuShopNextMilestoneBarGuidesImplementedSpine` | Tune target order and copy after playtesting; keep Net Worth valuation future-only | P1 | Parked/shop-only guidance; shows one milestone at a time; optional `$1T fictional Net Worth` line is a teaser only and does not add a counter, valuation, or driving effect |
| Automation | Counter Service V1 | Implemented | `counterService` shop state, `counterServiceIncomeStatus`, `counterServiceIntervalSeconds`, `startCounterService`, `pauseCounterService`, `applyCounterServiceTick`, `renderCounterServiceCard`, Counter Service station upgrades in `frontend/nospill/app.js` | `testCounterServiceV1AutomatesEarnedShopHandoffs`, `testCounterServicePolishStatsUpgradesAndSpiritPanel` | Playtest 10/8/6/4-second rate ladder and Best Available priority | P1 | Unlocks after First 10 Orders, starts paused, shows supplied/blocked income status, runs only while parked/open, does not auto-fulfill offline, and never affects Cup Test scoring or driving behavior |
...
| Shop | Covered-car long-term goal teaser | Implemented | `seenStoryBeatIds`, `queueStoryBeatTeaser`, `renderStoryTeaserCard`, first-upgrade purchase result | `testCoveredCarTeaserIsOneTimeStoryBeatOnly` | Decide later when to implement actual Dream Garage teaser/surface after first-loop tuning | P2 | Story beat only; no Dream Garage tab, parts, events, Builder Stars, Dream Jar, Project Car Value, or driving effects |
...
| Spirit | Shop Spirit and Festival Boosts | Placeholder | `SPIRIT_GENERATORS`, `SHOP_SPIRIT_BOOSTS`, `FESTIVAL_BOOSTS`, `renderSpiritPanel`, `useShopSpiritBoost`, `useFestivalBoost` | `testExpandedIdleShopLayerMechanics`, `testCounterServicePolishStatsUpgradesAndSpiritPanel` | Keep hidden/teased until stable production; add token earning rules later | P2 | Parked-only; panel shows Tips/Spirit wallet, classified Buy/Spend Spirit/Start Effect/Use Token actions, and no real driving score effect |
