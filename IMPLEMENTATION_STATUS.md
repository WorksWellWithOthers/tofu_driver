...
| Shop | Full progression design contract | Documented only | `BALANCE_AND_PROGRESSION.md` product role, resource contract, first-10-minute table, phase map, button contract, generator contract, upgrade ladder, bottleneck rules, reveal rules, License Exam design, balance sheet schema | `git diff --check` for doc pass | Implement and test the First Loop Contract before expanding advanced systems | P0 | Documents that Cup Test is optional and shop progression must not use speed/GPS/map/street/high-G data |
