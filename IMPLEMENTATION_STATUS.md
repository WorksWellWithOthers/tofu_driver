...
| Shop | Live generator ticking | Implemented | `tickOpenShopGenerators`, `applyShopGeneratorTick`, fractional `generatorCarry`, per-second shop rate labels | `testTofuShopStatePackIdleAndUpgradeRules`, `testLiveIdleTickAndShopButtonReliability` | Mobile pacing QA | P1 | Ticks locally every 1s while parked/open, saves periodically, no backend timer |
| Shop | Resource-funnel clarity | Implemented | `nextBestAction`, `currentBottleneck`, Orders panel reward copy, Tip-source disabled reasons | `testEarlyShopResourceFunnelMakesTipsObvious` | Continue first-session visual QA | P0 | Prioritizes shop order fulfillment over optional Cup Test when Tips are needed |
