...
| Passport | Expanded shop/passport stamps | Partial | `STAMP_LABELS`, first-order stamp result feedback, progressive Passport panel | `testExpandedIdleShopLayerMechanics`, `testTofuShopGeneratorUpgradeUiIsHonestAndProgressive`, `testShopOrderTypeProgressionAndRewards` | Category filters and details after first-loop tuning | P3 | Local-only achievements; first shop stamp is revealed without dumping the full catalog |
