...
| Shop | Resource-funnel clarity | Implemented | `nextBestAction`, `currentBottleneck`, Orders panel reward/prep copy, Tip-source disabled reasons | `testEarlyShopResourceFunnelMakesTipsObvious`, `testFractionalDeliveryOrdersShowPrepProgressNotRawDecimal` | Continue first-session visual QA | P0 | Prioritizes shop order fulfillment or Prep Counter waiting over optional Cup Test when the shop loop is active |
...
| Generator | Tofu Press station | Implemented | `SHOP_STATIONS`, `buyShopStation`, `renderShopGeneratorCard`, `getShopGeneratorRates` | `testLiveIdleTickAndShopButtonReliability`, `testTofuShopGeneratorUpgradeUiIsHonestAndProgressive` | Balance tuning | P1 | Produces Tofu Stock only; station ownership is separate from upgrade levels |
| Generator | Prep Counter station | Implemented | `SHOP_STATIONS`, `buyShopStation`, `renderShopGeneratorCard`, `getShopGeneratorRates` | `testLiveIdleTickAndShopButtonReliability`, `testTofuShopGeneratorUpgradeUiIsHonestAndProgressive` | Balance tuning | P1 | Converts Tofu Stock to Delivery Orders; station ownership is separate from upgrade levels |
