...
| External study | Antimatter Dimensions progression audit | Documented only | `EXTERNAL_REFERENCE_ANTIMATTER_DIMENSIONS_AUDIT.md` | `git diff --check` | Use only as future design reference; do not copy reference text, theme, formulas, UI, names, assets, or implementation details | Future | Translates progression architecture into safe Tofu Shop concepts while preserving no-speed/no-GPS/no-active-drive rules |
