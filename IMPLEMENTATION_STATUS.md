...
| Shop | First-loop balance contract | Implemented | Fresh state starts with 10 Tofu Stock, 1 ready Delivery Order, 1 Tofu Press, 1 Prep Counter; Simple Tofu Box tutorial order; First Shop Order stamp reveal; Steady Pressing costs 20 Tips and boosts Tofu Press output x1.5 | `testShopOrderTypeProgressionAndRewards`, `testNextBestActionHierarchyStaysSinglePrimary`, `testTofuShopGeneratorUpgradeUiIsHonestAndProgressive`, `testFractionalDeliveryOrdersShowPrepProgressNotRawDecimal` | `FIRST_LOOP_AUDIT.md` found the next tuning gap: first visible upgrade choice can miss the Prep Counter bottleneck, Upgrades can feel empty, and shop-order recent reward fallback can render `undefined` | P0 | Ordinary shop progression must not require real driving |
| Shop | First-loop audit | Documented only | `FIRST_LOOP_AUDIT.md` | `git diff --check` | Implement the first meaningful upgrade slice: fix shop recent reward fallback, align Prep Counter/Tidy Packaging recommendations, hide empty upgrade panels, and add upgrade before/after previews | P0 | Audit preserves local-first, parked-only shop scope and does not touch Cup Test scoring or speed behavior |
