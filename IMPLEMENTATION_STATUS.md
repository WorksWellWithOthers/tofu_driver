...
| Automation | Counter Service V1 | Implemented | `counterService` shop state, `startCounterService`, `pauseCounterService`, `applyCounterServiceTick`, `renderCounterServiceCard` in `frontend/nospill/app.js` | `testCounterServiceV1AutomatesEarnedShopHandoffs` | Playtest 10-second rate, Best Available priority, and future upgrade needs | P1 | Unlocks after First 10 Orders, starts paused, runs only while parked/open, does not auto-fulfill offline, and never affects Cup Test scoring or driving behavior |
