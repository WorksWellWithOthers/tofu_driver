...
`, context);
  ...
  assert(context.heatProgressCardHtml.includes('Next Dream Step: Showcase Finish · future'));
  assert(context.heatProgressCardHtml.includes('Showcase Finish comes in a future Dream Garage pass'));
  assert(context.heatProgressCardHtml.includes('Garage Build Value: $1.48M'));
  assert(context.heatCardHtml.includes('Heat Wrapped'));
  assert(context.heatCardHtml.includes('Exhaust Level 4 / 5'));
  assert(context.heatCardHtml.includes('The exhaust setup looks cared for, not rushed.'));
  assert(context.heatCardHtml.includes('Next Exhaust Work: Showcase Finish'));
  assert(context.heatCardHtml.includes('Full Dream Garage comes later'));
  assert(!context.heatCardHtml.includes('Showcase Finish</button>'));
  assert(!context.heatCardHtml.includes('Buy Showcase Finish'));
  assert(!context.heatCardHtml.includes('Heat Wrap Exhaust'));
  assert.strictEqual(context.heatDreamNote, 'Next Dream Step: Showcase Finish, future.');
  assert.notStrictEqual(context.heatAction.type, 'buy_dream_exhaust_work');
...
`, context);
  ...
  assert(html.includes('/static/nospill/app.js?v=20260618m'));
  assert(html.includes('/static/nospill/app.css?v=20260618m'));
}
