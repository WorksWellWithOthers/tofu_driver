...
globalThis.dreamBuildTiresLevel = dreamBuildTiresLevel;
globalThis.dreamBuildBrakesLevel = dreamBuildBrakesLevel;
...
globalThis.nextDreamBuildTiresWork = nextDreamBuildTiresWork;
globalThis.nextDreamBuildBrakesWork = nextDreamBuildBrakesWork;
...
globalThis.buyDreamBuildTires = buyDreamBuildTires;
globalThis.buyDreamBuildBrakes = buyDreamBuildBrakes;
...
`, context);
  ...
  assert(!context.freshOverviewHtml.includes('More status'));
  assert(!context.freshOverviewHtml.includes('Visible'));
  assert(!context.freshOverviewHtml.includes('data-details-key="overview_more_status"'));
  ...
  assert(context.afterFirstStampPassportHtml.includes('Overview'));
  assert(context.afterFirstStampPassportHtml.includes('Passport Stamp Found'));
  assert(!context.afterFirstStampPassportHtml.includes('More status'));
...
`, context);
  ...
  assert(!context.funnelOverviewHtml.includes('More status'));
  ...
  assert(!context.funnelOrdersFallbackHtml.includes('More status'));
...
`, context);
  ...
  assert(context.pausedOrdersHtml.includes('Preparing Next Order'));
  assert(!context.pausedOrdersHtml.includes('More status'));
  ...
  assert(context.highReadyOrdersHtml.includes('Preparing Next Order'));
  assert(!context.highReadyOrdersHtml.includes('More status'));
...
`, context);
  ...
  assert(context.highOrdersHtml.includes('Preparing Next Order'));
  assert(!context.highOrdersHtml.includes('More status'));
...
`, context);
  ...
  assert(context.compactPrepSlotsText.includes('slots open'));
...
`, context);
  ...
  assert.strictEqual(context.acknowledgedDreamProgressSummary.total, 40);
  ...
  assert(context.acknowledgedDreamProgressCard.includes('0 / 40 work stages'));
  ...
  assert.strictEqual(context.wheelsProgressSummary.total, 40);
  assert.strictEqual(context.wheelsProgressSummary.percent, 3);
  assert(context.wheelsProgressCardHtml.includes('Dream Build'));
  assert(context.wheelsProgressCardHtml.includes('1 / 40 work stages'));
  assert(context.wheelsProgressCardHtml.includes('aria-valuenow="1"'));
  assert(context.wheelsProgressCardHtml.includes('aria-valuemax="40"'));
  ...
  assert(context.wheelsProgressCardHtml.includes('Fictional Tofu Garage build value. Does not affect Cup Test scoring.'));
  ...
  assert.strictEqual(context.fitmentProgressSummary.total, 40);
  assert.strictEqual(context.fitmentProgressSummary.percent, 8);
  assert(context.fitmentProgressCardHtml.includes('3 / 40 work stages'));
  ...
  assert.strictEqual(context.exhaustProgressSummary.total, 40);
  assert(context.exhaustProgressCardHtml.includes('4 / 40 work stages'));
  ...
  assert.strictEqual(context.sealProgressSummary.total, 40);
  assert.strictEqual(context.sealProgressSummary.percent, 13);
  assert(context.sealProgressCardHtml.includes('Dream Build'));
  assert(context.sealProgressCardHtml.includes('5 / 40 work stages'));
  ...
  assert.strictEqual(context.sealDreamNote, 'Dream Build: 5 / 40 work stages complete.');
  ...
  assert.strictEqual(context.tunedProgressSummary.total, 40);
  assert.strictEqual(context.tunedProgressSummary.percent, 15);
  assert(context.tunedProgressCardHtml.includes('Dream Build'));
  assert(context.tunedProgressCardHtml.includes('6 / 40 work stages'));
  ...
  assert.strictEqual(context.tunedDreamNote, 'Dream Build: 6 / 40 work stages complete.');
  ...
  assert.strictEqual(context.heatProgressSummary.total, 40);
  assert.strictEqual(context.heatProgressSummary.percent, 18);
  assert(context.heatProgressCardHtml.includes('7 / 40 work stages'));
  ...
  assert.strictEqual(context.heatDreamNote, 'Dream Build: 7 / 40 work stages complete.');
  ...
  assert.strictEqual(context.showcaseFinishProgressSummary.total, 40);
  assert.strictEqual(context.showcaseFinishProgressSummary.percent, 20);
  assert(context.showcaseFinishProgressCardHtml.includes('8 / 40 work stages'));
...
const suspensionTwoReady = normalizeGameState(suspensionPurchase.gameState);
suspensionTwoReady.shop.tips = 8000000;
globalThis.suspensionTwoReadyCardHtml = renderDreamInvestmentTargetCard(suspensionTwoReady);
globalThis.suspensionTwoPinned = pinnedNearGoalForShop(suspensionTwoReady);
globalThis.suspensionTwoAction = nextBestAction(suspensionTwoReady);
globalThis.suspensionTwoPurchase = buyDreamBuildSuspension("ride-height-set", suspensionTwoReady, { activeDrive: false });
globalThis.suspensionTwoLevel = dreamBuildSuspensionLevel(suspensionTwoPurchase.gameState);
globalThis.suspensionTwoCashAfter = cashBalance(suspensionTwoPurchase.gameState);
globalThis.suspensionTwoProjectValue = projectCarValueV1(suspensionTwoPurchase.gameState);
globalThis.suspensionTwoProgress = dreamBuildProgressSummary(suspensionTwoPurchase.gameState);

const suspensionThreeReady = normalizeGameState(suspensionTwoPurchase.gameState);
suspensionThreeReady.shop.tips = 13000000;
globalThis.suspensionThreeReadyCardHtml = renderDreamInvestmentTargetCard(suspensionThreeReady);
globalThis.suspensionThreePinned = pinnedNearGoalForShop(suspensionThreeReady);
globalThis.suspensionThreePurchase = buyDreamBuildSuspension("alignment-dialed", suspensionThreeReady, { activeDrive: false });
globalThis.suspensionThreeLevel = dreamBuildSuspensionLevel(suspensionThreePurchase.gameState);
globalThis.suspensionThreeCashAfter = cashBalance(suspensionThreePurchase.gameState);
globalThis.suspensionThreeProjectValue = projectCarValueV1(suspensionThreePurchase.gameState);
globalThis.suspensionThreeProgress = dreamBuildProgressSummary(suspensionThreePurchase.gameState);

const suspensionFourReady = normalizeGameState(suspensionThreePurchase.gameState);
suspensionFourReady.shop.tips = 21000000;
globalThis.suspensionFourReadyCardHtml = renderDreamInvestmentTargetCard(suspensionFourReady);
globalThis.suspensionFourPinned = pinnedNearGoalForShop(suspensionFourReady);
globalThis.suspensionFourPurchase = buyDreamBuildSuspension("corner-balance", suspensionFourReady, { activeDrive: false });
globalThis.suspensionFourLevel = dreamBuildSuspensionLevel(suspensionFourPurchase.gameState);
globalThis.suspensionFourCashAfter = cashBalance(suspensionFourPurchase.gameState);
globalThis.suspensionFourProjectValue = projectCarValueV1(suspensionFourPurchase.gameState);
globalThis.suspensionFourProgress = dreamBuildProgressSummary(suspensionFourPurchase.gameState);

const suspensionFiveReady = normalizeGameState(suspensionFourPurchase.gameState);
suspensionFiveReady.shop.tips = 36000000;
globalThis.suspensionFiveReadyCardHtml = renderDreamInvestmentTargetCard(suspensionFiveReady);
globalThis.suspensionFivePinned = pinnedNearGoalForShop(suspensionFiveReady);
globalThis.suspensionFivePurchase = buyDreamBuildSuspension("showcase-stance", suspensionFiveReady, { activeDrive: false });
globalThis.suspensionFiveLevel = dreamBuildSuspensionLevel(suspensionFivePurchase.gameState);
globalThis.suspensionFiveCashAfter = cashBalance(suspensionFivePurchase.gameState);
globalThis.suspensionFiveProjectValue = projectCarValueV1(suspensionFivePurchase.gameState);
globalThis.suspensionFiveNetWorth = netWorthV1(suspensionFivePurchase.gameState);
globalThis.suspensionFiveFormulaNetWorth = cashBalance(suspensionFivePurchase.gameState) + tofuBusinessValue(suspensionFivePurchase.gameState) + projectCarValueV1(suspensionFivePurchase.gameState) + brandValueV1(suspensionFivePurchase.gameState);
globalThis.suspensionFiveProgress = dreamBuildProgressSummary(suspensionFivePurchase.gameState);
globalThis.suspensionFiveCardHtml = renderDreamInvestmentTargetCard(suspensionFivePurchase.gameState);
globalThis.suspensionFivePanelHtml = renderDreamBuildPanel(suspensionFivePurchase.gameState);
globalThis.suspensionFivePinnedAfter = pinnedNearGoalForShop(suspensionFivePurchase.gameState);
globalThis.suspensionFiveActionAfter = nextBestAction(suspensionFivePurchase.gameState);
globalThis.suspensionFiveSecondPurchase = buyDreamBuildSuspension("showcase-stance", suspensionFivePurchase.gameState, { activeDrive: false });

const suspensionFiveTick = normalizeGameState(suspensionFivePurchase.gameState);
suspensionFiveTick.shop.tips += 250000;
globalThis.suspensionFiveTickPinned = pinnedNearGoalForShop(suspensionFiveTick);

const tireOneReady = normalizeGameState(suspensionFivePurchase.gameState);
tireOneReady.shop.tips = 61000000;
globalThis.tireOneReadyCardHtml = renderDreamInvestmentTargetCard(tireOneReady);
globalThis.tireOnePinned = pinnedNearGoalForShop(tireOneReady);
globalThis.tireOneAction = nextBestAction(tireOneReady);
globalThis.tireOnePurchase = buyDreamBuildTires("sports-tire-set", tireOneReady, { activeDrive: false });
globalThis.tireOneLevel = dreamBuildTiresLevel(tireOnePurchase.gameState);
globalThis.tireOneCashAfter = cashBalance(tireOnePurchase.gameState);
globalThis.tireOneProjectValue = projectCarValueV1(tireOnePurchase.gameState);
globalThis.tireOneProgress = dreamBuildProgressSummary(tireOnePurchase.gameState);
globalThis.activeTirePurchase = buyDreamBuildTires("sports-tire-set", tireOneReady, { activeDrive: true });

const tireTwoReady = normalizeGameState(tireOnePurchase.gameState);
tireTwoReady.shop.tips = 91000000;
globalThis.tireTwoPurchase = buyDreamBuildTires("extreme-summer-tires", tireTwoReady, { activeDrive: false });
globalThis.tireTwoLevel = dreamBuildTiresLevel(tireTwoPurchase.gameState);
globalThis.tireTwoCashAfter = cashBalance(tireTwoPurchase.gameState);
globalThis.tireTwoProjectValue = projectCarValueV1(tireTwoPurchase.gameState);

const tireThreeReady = normalizeGameState(tireTwoPurchase.gameState);
tireThreeReady.shop.tips = 136000000;
globalThis.tireThreeReadyCardHtml = renderDreamInvestmentTargetCard(tireThreeReady);
globalThis.tireThreePurchase = buyDreamBuildTires("track-day-r-compounds", tireThreeReady, { activeDrive: false });
globalThis.tireThreeLevel = dreamBuildTiresLevel(tireThreePurchase.gameState);
globalThis.tireThreeCashAfter = cashBalance(tireThreePurchase.gameState);
globalThis.tireThreeProjectValue = projectCarValueV1(tireThreePurchase.gameState);

const tireFourReady = normalizeGameState(tireThreePurchase.gameState);
tireFourReady.shop.tips = 201000000;
globalThis.tireFourPurchase = buyDreamBuildTires("racing-slicks", tireFourReady, { activeDrive: false });
globalThis.tireFourLevel = dreamBuildTiresLevel(tireFourPurchase.gameState);
globalThis.tireFourCashAfter = cashBalance(tireFourPurchase.gameState);
globalThis.tireFourProjectValue = projectCarValueV1(tireFourPurchase.gameState);

const tireFiveReady = normalizeGameState(tireFourPurchase.gameState);
tireFiveReady.shop.tips = 301000000;
globalThis.tireFiveReadyCardHtml = renderDreamInvestmentTargetCard(tireFiveReady);
globalThis.tireFivePinned = pinnedNearGoalForShop(tireFiveReady);
globalThis.tireFivePurchase = buyDreamBuildTires("event-tire-set", tireFiveReady, { activeDrive: false });
globalThis.tireFiveLevel = dreamBuildTiresLevel(tireFivePurchase.gameState);
globalThis.tireFiveCashAfter = cashBalance(tireFivePurchase.gameState);
globalThis.tireFiveProjectValue = projectCarValueV1(tireFivePurchase.gameState);
globalThis.tireFiveProgress = dreamBuildProgressSummary(tireFivePurchase.gameState);
globalThis.tireFiveCardHtml = renderDreamInvestmentTargetCard(tireFivePurchase.gameState);
globalThis.tireFivePanelHtml = renderDreamBuildPanel(tireFivePurchase.gameState);
globalThis.tireFivePinnedAfter = pinnedNearGoalForShop(tireFivePurchase.gameState);
globalThis.tireFiveSecondPurchase = buyDreamBuildTires("event-tire-set", tireFivePurchase.gameState, { activeDrive: false });

const brakeOneReady = normalizeGameState(tireFivePurchase.gameState);
brakeOneReady.shop.tips = 451000000;
globalThis.brakeOneReadyCardHtml = renderDreamInvestmentTargetCard(brakeOneReady);
globalThis.brakeOnePinned = pinnedNearGoalForShop(brakeOneReady);
globalThis.brakeOneAction = nextBestAction(brakeOneReady);
globalThis.brakeOnePurchase = buyDreamBuildBrakes("sports-brake-pads", brakeOneReady, { activeDrive: false });
globalThis.brakeOneLevel = dreamBuildBrakesLevel(brakeOnePurchase.gameState);
globalThis.brakeOneCashAfter = cashBalance(brakeOnePurchase.gameState);
globalThis.brakeOneProjectValue = projectCarValueV1(brakeOnePurchase.gameState);
globalThis.brakeOneProgress = dreamBuildProgressSummary(brakeOnePurchase.gameState);
globalThis.activeBrakePurchase = buyDreamBuildBrakes("sports-brake-pads", brakeOneReady, { activeDrive: true });

const brakeTwoReady = normalizeGameState(brakeOnePurchase.gameState);
brakeTwoReady.shop.tips = 676000000;
globalThis.brakeTwoPurchase = buyDreamBuildBrakes("sports-brake-kit", brakeTwoReady, { activeDrive: false });
globalThis.brakeTwoLevel = dreamBuildBrakesLevel(brakeTwoPurchase.gameState);
globalThis.brakeTwoCashAfter = cashBalance(brakeTwoPurchase.gameState);
globalThis.brakeTwoProjectValue = projectCarValueV1(brakeTwoPurchase.gameState);

const brakeThreeReady = normalizeGameState(brakeTwoPurchase.gameState);
brakeThreeReady.shop.tips = 1001000000;
globalThis.brakeThreeReadyCardHtml = renderDreamInvestmentTargetCard(brakeThreeReady);
globalThis.brakeThreePurchase = buyDreamBuildBrakes("racing-brake-kit", brakeThreeReady, { activeDrive: false });
globalThis.brakeThreeLevel = dreamBuildBrakesLevel(brakeThreePurchase.gameState);
globalThis.brakeThreeCashAfter = cashBalance(brakeThreePurchase.gameState);
globalThis.brakeThreeProjectValue = projectCarValueV1(brakeThreePurchase.gameState);

const brakeFourReady = normalizeGameState(brakeThreePurchase.gameState);
brakeFourReady.shop.tips = 1501000000;
globalThis.brakeFourPurchase = buyDreamBuildBrakes("carbon-ceramic-big-brake-kit", brakeFourReady, { activeDrive: false });
globalThis.brakeFourLevel = dreamBuildBrakesLevel(brakeFourPurchase.gameState);
globalThis.brakeFourCashAfter = cashBalance(brakeFourPurchase.gameState);
globalThis.brakeFourProjectValue = projectCarValueV1(brakeFourPurchase.gameState);

const brakeFiveReady = normalizeGameState(brakeFourPurchase.gameState);
brakeFiveReady.shop.tips = 2251000000;
globalThis.brakeFiveReadyCardHtml = renderDreamInvestmentTargetCard(brakeFiveReady);
globalThis.brakeFivePinned = pinnedNearGoalForShop(brakeFiveReady);
globalThis.brakeFivePurchase = buyDreamBuildBrakes("brake-balance-control-package", brakeFiveReady, { activeDrive: false });
globalThis.brakeFiveLevel = dreamBuildBrakesLevel(brakeFivePurchase.gameState);
globalThis.brakeFiveCashAfter = cashBalance(brakeFivePurchase.gameState);
globalThis.brakeFiveProjectValue = projectCarValueV1(brakeFivePurchase.gameState);
globalThis.brakeFiveNetWorth = netWorthV1(brakeFivePurchase.gameState);
globalThis.brakeFiveFormulaNetWorth = cashBalance(brakeFivePurchase.gameState) + tofuBusinessValue(brakeFivePurchase.gameState) + projectCarValueV1(brakeFivePurchase.gameState) + brandValueV1(brakeFivePurchase.gameState);
globalThis.brakeFiveProgress = dreamBuildProgressSummary(brakeFivePurchase.gameState);
globalThis.brakeFiveCardHtml = renderDreamInvestmentTargetCard(brakeFivePurchase.gameState);
globalThis.brakeFivePanelHtml = renderDreamBuildPanel(brakeFivePurchase.gameState);
globalThis.brakeFivePinnedAfter = pinnedNearGoalForShop(brakeFivePurchase.gameState);
globalThis.brakeFiveActionAfter = nextBestAction(brakeFivePurchase.gameState);
globalThis.brakeFiveSecondPurchase = buyDreamBuildBrakes("brake-balance-control-package", brakeFivePurchase.gameState, { activeDrive: false });

const brakeFiveTick = normalizeGameState(brakeFivePurchase.gameState);
brakeFiveTick.shop.tips += 500000;
globalThis.brakeFiveTickPinned = pinnedNearGoalForShop(brakeFiveTick);

const inductionNoShowcase = normalizeGameState(brakeFivePurchase.gameState);
inductionNoShowcase.shop.tips = 3501000000;
globalThis.inductionNoShowcaseCardHtml = renderDreamInvestmentTargetCard(inductionNoShowcase);
globalThis.inductionNoShowcaseResult = buyDreamBuildInduction("sports-intercooler", inductionNoShowcase, { activeDrive: false });

const inductionShowcaseSeed = normalizeGameState(brakeFivePurchase.gameState);
inductionShowcaseSeed.shop.tips = 100000000;
globalThis.inductionShowcaseResult = completeGarageEvent("local_showcase", inductionShowcaseSeed, { activeDrive: false, now: "2026-06-19T12:00:00.000Z" });

const inductionOneReady = normalizeGameState(inductionShowcaseResult.gameState);
inductionOneReady.shop.tips = 3501000000;
globalThis.inductionOneReadyCardHtml = renderDreamInvestmentTargetCard(inductionOneReady);
globalThis.inductionOnePinned = pinnedNearGoalForShop(inductionOneReady);
globalThis.inductionOneAction = nextBestAction(inductionOneReady);
globalThis.inductionOnePurchase = buyDreamBuildInduction("sports-intercooler", inductionOneReady, { activeDrive: false });
globalThis.inductionOneLevel = dreamBuildInductionLevel(inductionOnePurchase.gameState);
globalThis.inductionOneCashAfter = cashBalance(inductionOnePurchase.gameState);
globalThis.inductionOneProjectValue = projectCarValueV1(inductionOnePurchase.gameState);
globalThis.inductionOneProgress = dreamBuildProgressSummary(inductionOnePurchase.gameState);
globalThis.activeInductionPurchase = buyDreamBuildInduction("sports-intercooler", inductionOneReady, { activeDrive: true });

const inductionTwoReady = normalizeGameState(inductionOnePurchase.gameState);
inductionTwoReady.shop.tips = 5251000000;
globalThis.inductionTwoPurchase = buyDreamBuildInduction("electronic-boost-control", inductionTwoReady, { activeDrive: false });
globalThis.inductionTwoLevel = dreamBuildInductionLevel(inductionTwoPurchase.gameState);
globalThis.inductionTwoCashAfter = cashBalance(inductionTwoPurchase.gameState);
globalThis.inductionTwoProjectValue = projectCarValueV1(inductionTwoPurchase.gameState);

const inductionThreeReady = normalizeGameState(inductionTwoPurchase.gameState);
inductionThreeReady.shop.tips = 8001000000;
globalThis.inductionThreeReadyCardHtml = renderDreamInvestmentTargetCard(inductionThreeReady);
globalThis.inductionThreePurchase = buyDreamBuildInduction("hybrid-turbo-upgrade", inductionThreeReady, { activeDrive: false });
globalThis.inductionThreeLevel = dreamBuildInductionLevel(inductionThreePurchase.gameState);
globalThis.inductionThreeCashAfter = cashBalance(inductionThreePurchase.gameState);
globalThis.inductionThreeProjectValue = projectCarValueV1(inductionThreePurchase.gameState);

const inductionFourReady = normalizeGameState(inductionThreePurchase.gameState);
inductionFourReady.shop.tips = 12001000000;
globalThis.inductionFourPurchase = buyDreamBuildInduction("big-turbo-kit", inductionFourReady, { activeDrive: false });
globalThis.inductionFourLevel = dreamBuildInductionLevel(inductionFourPurchase.gameState);
globalThis.inductionFourCashAfter = cashBalance(inductionFourPurchase.gameState);
globalThis.inductionFourProjectValue = projectCarValueV1(inductionFourPurchase.gameState);

const inductionFiveReady = normalizeGameState(inductionFourPurchase.gameState);
inductionFiveReady.shop.tips = 18001000000;
globalThis.inductionFiveReadyCardHtml = renderDreamInvestmentTargetCard(inductionFiveReady);
globalThis.inductionFivePinned = pinnedNearGoalForShop(inductionFiveReady);
globalThis.inductionFivePurchase = buyDreamBuildInduction("anti-lag-cooling-package", inductionFiveReady, { activeDrive: false });
globalThis.inductionFiveLevel = dreamBuildInductionLevel(inductionFivePurchase.gameState);
globalThis.inductionFiveCashAfter = cashBalance(inductionFivePurchase.gameState);
globalThis.inductionFiveProjectValue = projectCarValueV1(inductionFivePurchase.gameState);
globalThis.inductionFiveNetWorth = netWorthV1(inductionFivePurchase.gameState);
globalThis.inductionFiveFormulaNetWorth = cashBalance(inductionFivePurchase.gameState) + tofuBusinessValue(inductionFivePurchase.gameState) + projectCarValueV1(inductionFivePurchase.gameState) + brandValueV1(inductionFivePurchase.gameState);
globalThis.inductionFiveProgress = dreamBuildProgressSummary(inductionFivePurchase.gameState);
globalThis.inductionFiveCardHtml = renderDreamInvestmentTargetCard(inductionFivePurchase.gameState);
globalThis.inductionFivePanelHtml = renderDreamBuildPanel(inductionFivePurchase.gameState);
globalThis.inductionFivePinnedAfter = pinnedNearGoalForShop(inductionFivePurchase.gameState);
globalThis.inductionFiveActionAfter = nextBestAction(inductionFivePurchase.gameState);
globalThis.inductionFiveSecondPurchase = buyDreamBuildInduction("anti-lag-cooling-package", inductionFivePurchase.gameState, { activeDrive: false });

const inductionFiveTick = normalizeGameState(inductionFivePurchase.gameState);
inductionFiveTick.shop.tips += 500000;
globalThis.inductionFiveTickPinned = pinnedNearGoalForShop(inductionFiveTick);

const drivetrainOneReady = normalizeGameState(inductionFivePurchase.gameState);
drivetrainOneReady.shop.tips = 27001000000;
globalThis.drivetrainOneReadyCardHtml = renderDreamInvestmentTargetCard(drivetrainOneReady);
globalThis.drivetrainOnePinned = pinnedNearGoalForShop(drivetrainOneReady);
globalThis.drivetrainOneAction = nextBestAction(drivetrainOneReady);
globalThis.drivetrainOnePurchase = buyDreamBuildDrivetrain("sports-clutch-flywheel", drivetrainOneReady, { activeDrive: false });
globalThis.drivetrainOneLevel = dreamBuildDrivetrainLevel(drivetrainOnePurchase.gameState);
globalThis.drivetrainOneCashAfter = cashBalance(drivetrainOnePurchase.gameState);
globalThis.drivetrainOneProjectValue = projectCarValueV1(drivetrainOnePurchase.gameState);
globalThis.drivetrainOneProgress = dreamBuildProgressSummary(drivetrainOnePurchase.gameState);
globalThis.activeDrivetrainPurchase = buyDreamBuildDrivetrain("sports-clutch-flywheel", drivetrainOneReady, { activeDrive: true });

const drivetrainTwoReady = normalizeGameState(drivetrainOnePurchase.gameState);
drivetrainTwoReady.shop.tips = 40001000000;
globalThis.drivetrainTwoReadyCardHtml = renderDreamInvestmentTargetCard(drivetrainTwoReady);
globalThis.drivetrainTwoPurchase = buyDreamBuildDrivetrain("limited-slip-differential", drivetrainTwoReady, { activeDrive: false });
globalThis.drivetrainTwoLevel = dreamBuildDrivetrainLevel(drivetrainTwoPurchase.gameState);
globalThis.drivetrainTwoCashAfter = cashBalance(drivetrainTwoPurchase.gameState);
globalThis.drivetrainTwoProjectValue = projectCarValueV1(drivetrainTwoPurchase.gameState);
globalThis.drivetrainTwoProgress = dreamBuildProgressSummary(drivetrainTwoPurchase.gameState);

const drivetrainThreeReady = normalizeGameState(drivetrainTwoPurchase.gameState);
drivetrainThreeReady.shop.tips = 60001000000;
globalThis.drivetrainThreeReadyCardHtml = renderDreamInvestmentTargetCard(drivetrainThreeReady);
globalThis.drivetrainThreePinned = pinnedNearGoalForShop(drivetrainThreeReady);
globalThis.drivetrainThreePurchase = buyDreamBuildDrivetrain("carbon-driveshaft-axles", drivetrainThreeReady, { activeDrive: false });
globalThis.drivetrainThreeLevel = dreamBuildDrivetrainLevel(drivetrainThreePurchase.gameState);
globalThis.drivetrainThreeCashAfter = cashBalance(drivetrainThreePurchase.gameState);
globalThis.drivetrainThreeProjectValue = projectCarValueV1(drivetrainThreePurchase.gameState);
globalThis.drivetrainThreeProgress = dreamBuildProgressSummary(drivetrainThreePurchase.gameState);

const drivetrainFourReady = normalizeGameState(drivetrainThreePurchase.gameState);
drivetrainFourReady.shop.tips = 90001000000;
globalThis.drivetrainFourReadyCardHtml = renderDreamInvestmentTargetCard(drivetrainFourReady);
globalThis.drivetrainFourPinned = pinnedNearGoalForShop(drivetrainFourReady);
globalThis.drivetrainFourPurchase = buyDreamBuildDrivetrain("custom-gearbox", drivetrainFourReady, { activeDrive: false });
globalThis.drivetrainFourLevel = dreamBuildDrivetrainLevel(drivetrainFourPurchase.gameState);
globalThis.drivetrainFourCashAfter = cashBalance(drivetrainFourPurchase.gameState);
globalThis.drivetrainFourProjectValue = projectCarValueV1(drivetrainFourPurchase.gameState);
globalThis.drivetrainFourProgress = dreamBuildProgressSummary(drivetrainFourPurchase.gameState);

const drivetrainFiveReady = normalizeGameState(drivetrainFourPurchase.gameState);
drivetrainFiveReady.shop.tips = 135001000000;
globalThis.drivetrainFiveReadyCardHtml = renderDreamInvestmentTargetCard(drivetrainFiveReady);
globalThis.drivetrainFivePinned = pinnedNearGoalForShop(drivetrainFiveReady);
globalThis.drivetrainFivePurchase = buyDreamBuildDrivetrain("sequential-transmission-package", drivetrainFiveReady, { activeDrive: false });
globalThis.drivetrainFiveLevel = dreamBuildDrivetrainLevel(drivetrainFivePurchase.gameState);
globalThis.drivetrainFiveCashAfter = cashBalance(drivetrainFivePurchase.gameState);
globalThis.drivetrainFiveProjectValue = projectCarValueV1(drivetrainFivePurchase.gameState);
globalThis.drivetrainFiveNetWorth = netWorthV1(drivetrainFivePurchase.gameState);
globalThis.drivetrainFiveFormulaNetWorth = cashBalance(drivetrainFivePurchase.gameState) + tofuBusinessValue(drivetrainFivePurchase.gameState) + projectCarValueV1(drivetrainFivePurchase.gameState) + brandValueV1(drivetrainFivePurchase.gameState);
globalThis.drivetrainFiveProgress = dreamBuildProgressSummary(drivetrainFivePurchase.gameState);
globalThis.drivetrainFiveCardHtml = renderDreamInvestmentTargetCard(drivetrainFivePurchase.gameState);
globalThis.drivetrainFivePanelHtml = renderDreamBuildPanel(drivetrainFivePurchase.gameState);
globalThis.drivetrainFivePinnedAfter = pinnedNearGoalForShop(drivetrainFivePurchase.gameState);
globalThis.drivetrainFiveActionAfter = nextBestAction(drivetrainFivePurchase.gameState);
globalThis.drivetrainFiveSecondPurchase = buyDreamBuildDrivetrain("sequential-transmission-package", drivetrainFivePurchase.gameState, { activeDrive: false });

const drivetrainFiveTick = normalizeGameState(drivetrainFivePurchase.gameState);
drivetrainFiveTick.shop.tips += 500000;
globalThis.drivetrainFiveTickPinned = pinnedNearGoalForShop(drivetrainFiveTick);

const aeroOneReady = normalizeGameState(drivetrainFivePurchase.gameState);
aeroOneReady.shop.tips = 200001000000;
globalThis.aeroOneReadyCardHtml = renderDreamInvestmentTargetCard(aeroOneReady);
globalThis.aeroOnePinned = pinnedNearGoalForShop(aeroOneReady);
globalThis.aeroOneAction = nextBestAction(aeroOneReady);
globalThis.aeroOnePurchase = buyDreamBuildAero("front-splitter-side-skirts", aeroOneReady, { activeDrive: false });
globalThis.aeroOneLevel = dreamBuildAeroLevel(aeroOnePurchase.gameState);
globalThis.aeroOneCashAfter = cashBalance(aeroOnePurchase.gameState);
globalThis.aeroOneProjectValue = projectCarValueV1(aeroOnePurchase.gameState);
globalThis.aeroOneProgress = dreamBuildProgressSummary(aeroOnePurchase.gameState);
globalThis.activeAeroPurchase = buyDreamBuildAero("front-splitter-side-skirts", aeroOneReady, { activeDrive: true });

const aeroTwoReady = normalizeGameState(aeroOnePurchase.gameState);
aeroTwoReady.shop.tips = 300001000000;
globalThis.aeroTwoReadyCardHtml = renderDreamInvestmentTargetCard(aeroTwoReady);
globalThis.aeroTwoPurchase = buyDreamBuildAero("rear-diffuser-wing", aeroTwoReady, { activeDrive: false });
globalThis.aeroTwoLevel = dreamBuildAeroLevel(aeroTwoPurchase.gameState);
globalThis.aeroTwoCashAfter = cashBalance(aeroTwoPurchase.gameState);
globalThis.aeroTwoProjectValue = projectCarValueV1(aeroTwoPurchase.gameState);
globalThis.aeroTwoProgress = dreamBuildProgressSummary(aeroTwoPurchase.gameState);

const aeroThreeReady = normalizeGameState(aeroTwoPurchase.gameState);
aeroThreeReady.shop.tips = 450001000000;
globalThis.aeroThreeReadyCardHtml = renderDreamInvestmentTargetCard(aeroThreeReady);
globalThis.aeroThreePinned = pinnedNearGoalForShop(aeroThreeReady);
globalThis.aeroThreePurchase = buyDreamBuildAero("wide-body-vented-panels", aeroThreeReady, { activeDrive: false });
globalThis.aeroThreeLevel = dreamBuildAeroLevel(aeroThreePurchase.gameState);
globalThis.aeroThreeCashAfter = cashBalance(aeroThreePurchase.gameState);
globalThis.aeroThreeProjectValue = projectCarValueV1(aeroThreePurchase.gameState);
globalThis.aeroThreeProgress = dreamBuildProgressSummary(aeroThreePurchase.gameState);
globalThis.aeroOverviewSummary = renderOverviewPanel(aeroThreePurchase.gameState);

const aeroFourReady = normalizeGameState(aeroThreePurchase.gameState);
aeroFourReady.shop.tips = 675001000000;
globalThis.aeroFourReadyCardHtml = renderDreamInvestmentTargetCard(aeroFourReady);
globalThis.aeroFourPinned = pinnedNearGoalForShop(aeroFourReady);
globalThis.aeroFourPurchase = buyDreamBuildAero("weight-reduction-package", aeroFourReady, { activeDrive: false });
globalThis.aeroFourLevel = dreamBuildAeroLevel(aeroFourPurchase.gameState);
globalThis.aeroFourCashAfter = cashBalance(aeroFourPurchase.gameState);
globalThis.aeroFourProjectValue = projectCarValueV1(aeroFourPurchase.gameState);
globalThis.aeroFourProgress = dreamBuildProgressSummary(aeroFourPurchase.gameState);

const aeroFiveReady = normalizeGameState(aeroFourPurchase.gameState);
aeroFiveReady.shop.tips = 1000001000000;
globalThis.aeroFiveReadyCardHtml = renderDreamInvestmentTargetCard(aeroFiveReady);
globalThis.aeroFivePinned = pinnedNearGoalForShop(aeroFiveReady);
globalThis.aeroFivePurchase = buyDreamBuildAero("carbon-body-roll-cage", aeroFiveReady, { activeDrive: false });
globalThis.aeroFiveLevel = dreamBuildAeroLevel(aeroFivePurchase.gameState);
globalThis.aeroFiveCashAfter = cashBalance(aeroFivePurchase.gameState);
globalThis.aeroFiveProjectValue = projectCarValueV1(aeroFivePurchase.gameState);
globalThis.aeroFiveNetWorth = netWorthV1(aeroFivePurchase.gameState);
globalThis.aeroFiveFormulaNetWorth = cashBalance(aeroFivePurchase.gameState) + tofuBusinessValue(aeroFivePurchase.gameState) + projectCarValueV1(aeroFivePurchase.gameState) + brandValueV1(aeroFivePurchase.gameState);
globalThis.aeroFiveProgress = dreamBuildProgressSummary(aeroFivePurchase.gameState);
globalThis.aeroFiveCardHtml = renderDreamInvestmentTargetCard(aeroFivePurchase.gameState);
globalThis.aeroFivePanelHtml = renderDreamBuildPanel(aeroFivePurchase.gameState);
globalThis.aeroFivePinnedAfter = pinnedNearGoalForShop(aeroFivePurchase.gameState);
globalThis.aeroFiveActionAfter = nextBestAction(aeroFivePurchase.gameState);
globalThis.aeroFiveSecondPurchase = buyDreamBuildAero("carbon-body-roll-cage", aeroFivePurchase.gameState, { activeDrive: false });

const aeroFiveTick = normalizeGameState(aeroFivePurchase.gameState);
...
`, context);
  ...
  assert(context.dreamWheelsPanelHtml.includes('Current Build'));
  assert(context.dreamWheelsPanelHtml.includes('Core Build Progress'));
  assert(context.dreamWheelsPanelHtml.includes('Next Work: Polish Wheels'));
  assert(context.dreamWheelsPanelHtml.includes('Work Tracks'));
  assert(context.dreamWheelsPanelHtml.includes('Track details'));
  assert(context.dreamWheelsPanelHtml.includes('Garage Events'));
  assert(context.dreamWheelsPanelHtml.includes('Future Garage'));
  assert(context.dreamWheelsPanelHtml.includes('data-details-key="dream_build_garage_events"'));
  assert(context.dreamWheelsPanelHtml.includes('data-details-key="dream_build_future_management"'));
  assert(context.dreamWheelsPanelHtml.indexOf('Next Work: Polish Wheels') < context.dreamWheelsPanelHtml.indexOf('Track details'));
  ...
  assert(context.overviewOfflineFooterText.includes('Ledger'));
  assert(!context.overviewOfflineFooterText.includes('View Ledger'));
  assert(context.overviewOfflineFooterHtml.includes('nospill-inline-ledger'));
  ...
  assert.strictEqual(context.suspensionLowPinned.title, 'Suspension Refreshed');
  assert.strictEqual(context.suspensionLowTickPinned.title, 'Suspension Refreshed');
  ...
  assert.strictEqual(context.suspensionReadyPinned.title, 'Suspension Refreshed');
  ...
  assert.strictEqual(context.suspensionProgressAfter.total, 40);
  ...
  assert(context.suspensionAfterCardHtml.includes('Cost: $7.5M Cash'));
  assert(context.suspensionAfterCardHtml.includes('Build Value added: +$4M'));
  assert(context.suspensionAfterCardHtml.includes('Need $6.5M more Cash.'));
  assert(!context.suspensionAfterCardHtml.includes('Set Ride Height</button>'));
  assert.strictEqual(context.suspensionAfterPinned.id, 'ride-height-set');
  assert.strictEqual(context.suspensionAfterPinned.title, 'Ride Height Set');
  assert.strictEqual(context.suspensionAfterAction.title, 'Next: Grow Cash for Ride Height Set');
  assert.strictEqual(context.secondSuspensionPurchase.ok, false);
  assert.strictEqual(context.activeSuspensionPurchase.ok, false);

  assert(context.suspensionTwoReadyCardHtml.includes('Set Ride Height'));
  assert(context.suspensionTwoReadyCardHtml.includes('height-adjustable sports suspension'));
  assert(context.suspensionTwoReadyCardHtml.includes('Suspension details'));
  assert.strictEqual(context.suspensionTwoPinned.title, 'Ride Height Set');
  assert.strictEqual(context.suspensionTwoAction.buttonLabel, 'Set Ride Height');
  assert.strictEqual(context.suspensionTwoPurchase.ok, true);
  assert.strictEqual(context.suspensionTwoLevel, 2);
  assert.strictEqual(context.suspensionTwoCashAfter, 500000);
  assert.strictEqual(context.suspensionTwoProjectValue, 8725000);
  assert.strictEqual(context.suspensionTwoProgress.completed, 10);

  assert(context.suspensionThreeReadyCardHtml.includes('Dial Alignment'));
  assert(context.suspensionThreeReadyCardHtml.includes('Cost: $12M Cash'));
  assert(context.suspensionThreeReadyCardHtml.includes('Build Value added: +$7M'));
  assert(context.suspensionThreeReadyCardHtml.includes('camber/toe arms'));
  assert.strictEqual(context.suspensionThreePinned.title, 'Alignment Dialed');
  assert.strictEqual(context.suspensionThreePurchase.ok, true);
  assert.strictEqual(context.suspensionThreeLevel, 3);
  assert.strictEqual(context.suspensionThreeCashAfter, 1000000);
  assert.strictEqual(context.suspensionThreeProjectValue, 15725000);
  assert.strictEqual(context.suspensionThreeProgress.completed, 11);

  assert(context.suspensionFourReadyCardHtml.includes('Corner Balance'));
  assert(context.suspensionFourReadyCardHtml.includes('Cost: $20M Cash'));
  assert(context.suspensionFourReadyCardHtml.includes('Build Value added: +$12M'));
  assert(context.suspensionFourReadyCardHtml.includes('fully customizable coilovers'));
  assert.strictEqual(context.suspensionFourPinned.title, 'Corner Balance');
  assert.strictEqual(context.suspensionFourPurchase.ok, true);
  assert.strictEqual(context.suspensionFourLevel, 4);
  assert.strictEqual(context.suspensionFourCashAfter, 1000000);
  assert.strictEqual(context.suspensionFourProjectValue, 27725000);
  assert.strictEqual(context.suspensionFourProgress.completed, 12);

  assert(context.suspensionFiveReadyCardHtml.includes('Finish Suspension'));
  assert(context.suspensionFiveReadyCardHtml.includes('Cost: $35M Cash'));
  assert(context.suspensionFiveReadyCardHtml.includes('Build Value added: +$22M'));
  assert(context.suspensionFiveReadyCardHtml.includes('Final geometry, bracing, fitment'));
  assert.strictEqual(context.suspensionFivePinned.title, 'Showcase Stance');
  assert.strictEqual(context.suspensionFivePurchase.ok, true);
  assert.strictEqual(context.suspensionFivePurchase.feedback, 'Suspension complete: Showcase Stance added. Garage Build Value +$22M.');
  assert.strictEqual(context.suspensionFiveLevel, 5);
  assert.strictEqual(context.suspensionFiveCashAfter, 1000000);
  assert.strictEqual(context.suspensionFiveProjectValue, 49725000);
  assert(context.suspensionFiveNetWorth <= context.suspensionFiveFormulaNetWorth);
  assert(context.suspensionFiveNetWorth >= context.suspensionFiveProjectValue);
  assert.strictEqual(context.suspensionFiveProgress.completed, 13);
  assert.strictEqual(context.suspensionFiveProgress.total, 40);
  assert(context.suspensionFiveCardHtml.includes('Tires &amp; Rubber'));
  assert(context.suspensionFiveCardHtml.includes('Level 0 / 5 · Stock Tires'));
  assert(context.suspensionFiveCardHtml.includes('Next Work: Sports Tire Set'));
  assert(context.suspensionFiveCardHtml.includes('Cost: $60M Cash'));
  assert(context.suspensionFiveCardHtml.includes('Build Value added: +$35M'));
  assert(context.suspensionFiveCardHtml.includes('Need $59M more Cash.'));
  assert(!context.suspensionFiveCardHtml.includes('Fit Sports Tires</button>'));
  assert.strictEqual(context.suspensionFivePinnedAfter.id, 'sports-tire-set');
  assert.strictEqual(context.suspensionFivePinnedAfter.title, 'Sports Tire Set');
  assert(context.suspensionFivePinnedAfter.body.includes('Next build track is ready.'));
  assert.strictEqual(context.suspensionFiveTickPinned.title, 'Sports Tire Set');
  assert.strictEqual(context.suspensionFiveActionAfter.title, 'Next: Grow Cash for Sports Tire Set');
  assert.strictEqual(context.suspensionFiveSecondPurchase.ok, false);

  assert(context.suspensionFivePanelHtml.includes('Current Build'));
  assert(context.suspensionFivePanelHtml.includes('Garage Build Value'));
  assert(context.suspensionFivePanelHtml.includes('Builder Note'));
  assert(context.suspensionFivePanelHtml.includes('Garage Tuning Catalog'));
  assert(context.suspensionFivePanelHtml.includes('Current focus: Tires &amp; Rubber.'));
  assert(context.suspensionFivePanelHtml.includes('<summary>Catalog categories</summary>'));
  assert(context.suspensionFivePanelHtml.includes('Tires &amp; Rubber'));
  assert(context.suspensionFivePanelHtml.includes('Suspension &amp; Chassis Geometry'));
  assert(context.suspensionFivePanelHtml.includes('Brakes &amp; Control'));
  assert(context.suspensionFivePanelHtml.includes('Airflow, Intake &amp; Exhaust'));
  assert(context.suspensionFivePanelHtml.includes('Utility, Restorations &amp; Swaps'));
  ...
  assert(context.tireOneReadyCardHtml.includes('Fit Sports Tires'));
  assert(context.tireOneReadyCardHtml.includes('Cost: $60M Cash'));
  assert(context.tireOneReadyCardHtml.includes('Build Value added: +$35M'));
  assert(context.tireOneReadyCardHtml.includes('Sports tire compounds'));
  assert.strictEqual(context.tireOnePinned.title, 'Sports Tire Set');
  assert.strictEqual(context.tireOneAction.type, 'buy_dream_tires_work');
  assert.strictEqual(context.tireOnePurchase.ok, true);
  assert.strictEqual(context.tireOneLevel, 1);
  assert.strictEqual(context.tireOneCashAfter, 1000000);
  assert.strictEqual(context.tireOneProjectValue, 84725000);
  assert.strictEqual(context.tireOneProgress.completed, 14);
  assert.strictEqual(context.tireOneProgress.total, 40);
  assert.strictEqual(context.activeTirePurchase.ok, false);
  assert.strictEqual(context.tireTwoPurchase.ok, true);
  assert.strictEqual(context.tireTwoLevel, 2);
  assert.strictEqual(context.tireTwoCashAfter, 1000000);
  assert.strictEqual(context.tireTwoProjectValue, 139725000);
  assert(context.tireThreeReadyCardHtml.includes('Fit R-Compounds'));
  assert(context.tireThreeReadyCardHtml.includes('Cost: $135M Cash'));
  assert(context.tireThreeReadyCardHtml.includes('Build Value added: +$85M'));
  assert.strictEqual(context.tireThreePurchase.ok, true);
  assert.strictEqual(context.tireThreeLevel, 3);
  assert.strictEqual(context.tireThreeCashAfter, 1000000);
  assert.strictEqual(context.tireThreeProjectValue, 224725000);
  assert.strictEqual(context.tireFourPurchase.ok, true);
  assert.strictEqual(context.tireFourLevel, 4);
  assert.strictEqual(context.tireFourCashAfter, 1000000);
  assert.strictEqual(context.tireFourProjectValue, 354725000);
  assert(context.tireFiveReadyCardHtml.includes('Complete Tire Set'));
  assert(context.tireFiveReadyCardHtml.includes('Cost: $300M Cash'));
  assert(context.tireFiveReadyCardHtml.includes('Build Value added: +$200M'));
  assert.strictEqual(context.tireFivePinned.title, 'Event Tire Set');
  assert.strictEqual(context.tireFivePurchase.ok, true);
  assert.strictEqual(context.tireFiveLevel, 5);
  assert.strictEqual(context.tireFiveCashAfter, 1000000);
  assert.strictEqual(context.tireFiveProjectValue, 554725000);
  assert.strictEqual(context.tireFiveProgress.completed, 18);
  assert.strictEqual(context.tireFiveProgress.total, 40);
  assert(context.tireFiveCardHtml.includes('Brakes &amp; Control'));
  assert(context.tireFiveCardHtml.includes('Next Work: Sports Brake Pads'));
  ...
  assert.strictEqual(context.brakeOneProjectValue, 854725000);
  assert.strictEqual(context.brakeOneProgress.completed, 19);
  assert.strictEqual(context.activeBrakePurchase.ok, false);
  assert.strictEqual(context.brakeTwoPurchase.ok, true);
  assert.strictEqual(context.brakeTwoLevel, 2);
  assert.strictEqual(context.brakeTwoCashAfter, 1000000);
  assert.strictEqual(context.brakeTwoProjectValue, 1304725000);
  assert(context.brakeThreeReadyCardHtml.includes('Install Racing Brakes'));
  assert(context.brakeThreeReadyCardHtml.includes('Cost: $1B Cash'));
  assert(context.brakeThreeReadyCardHtml.includes('Build Value added: +$700M'));
  assert.strictEqual(context.brakeThreePurchase.ok, true);
  assert.strictEqual(context.brakeThreeLevel, 3);
  assert.strictEqual(context.brakeThreeCashAfter, 1000000);
  assert.strictEqual(context.brakeThreeProjectValue, 2004725000);
  assert.strictEqual(context.brakeFourPurchase.ok, true);
  assert.strictEqual(context.brakeFourLevel, 4);
  assert.strictEqual(context.brakeFourCashAfter, 1000000);
  assert.strictEqual(context.brakeFourProjectValue, 3054725000);
  ...
  assert.strictEqual(context.brakeFiveProjectValue, 4604725000);
  assert(context.brakeFiveNetWorth <= context.brakeFiveFormulaNetWorth);
  assert(context.brakeFiveNetWorth >= context.brakeFiveProjectValue);
  assert.strictEqual(context.brakeFiveProgress.completed, 23);
  assert.strictEqual(context.brakeFiveProgress.total, 40);
  assert(context.brakeFiveCardHtml.includes('Induction &amp; Cooling'));
  assert(context.brakeFiveCardHtml.includes('Future Track'));
  assert(context.brakeFiveCardHtml.includes('Enter the Local Showcase'));
  assert(context.brakeFivePanelHtml.includes('Brakes &amp; Control'));
  assert(context.brakeFivePanelHtml.includes('Complete · Next Build Track: Induction &amp; Cooling.'));
  assert(context.brakeFivePanelHtml.includes('Current focus: Induction &amp; Cooling.'));
  assert(!context.brakeFivePanelHtml.includes('Buy Induction'));
  assert(!context.brakeFivePanelHtml.includes('Install Induction'));
  assert.strictEqual(context.brakeFivePinnedAfter.id, 'local_showcase_for_induction');
  assert.strictEqual(context.brakeFivePinnedAfter.title, 'Enter Local Showcase');
  assert.strictEqual(context.brakeFiveTickPinned.title, 'Enter Local Showcase');
  ...
  assert(context.inductionNoShowcaseCardHtml.includes('Induction &amp; Cooling'));
  assert(context.inductionNoShowcaseCardHtml.includes('Future Track'));
  assert(context.inductionNoShowcaseCardHtml.includes('Enter the Local Showcase'));
  assert(!context.inductionNoShowcaseCardHtml.includes('Install Intercooler</button>'));
  assert.strictEqual(context.inductionNoShowcaseResult.ok, false);
  assert(context.inductionNoShowcaseResult.reason.includes('Local Showcase'));

  assert.strictEqual(context.inductionShowcaseResult.ok, true);
  assert(context.inductionOneReadyCardHtml.includes('Install Intercooler'));
  assert(context.inductionOneReadyCardHtml.includes('Cost: $3.5B Cash'));
  assert(context.inductionOneReadyCardHtml.includes('Build Value added: +$2.5B'));
  assert(context.inductionOneReadyCardHtml.includes('Sports / racing intercooler language'));
  assert.strictEqual(context.inductionOnePinned.title, 'Sports Intercooler');
  assert.strictEqual(context.inductionOneAction.type, 'buy_dream_induction_work');
  assert.strictEqual(context.inductionOnePurchase.ok, true);
  assert.strictEqual(context.inductionOneLevel, 1);
  assert.strictEqual(context.inductionOneCashAfter, 1000000);
  assert.strictEqual(context.inductionOneProjectValue, 7104725000);
  assert.strictEqual(context.inductionOneProgress.completed, 24);
  assert.strictEqual(context.inductionOneProgress.total, 40);
  assert.strictEqual(context.activeInductionPurchase.ok, false);

  assert.strictEqual(context.inductionTwoPurchase.ok, true);
  assert.strictEqual(context.inductionTwoLevel, 2);
  assert.strictEqual(context.inductionTwoCashAfter, 1000000);
  assert.strictEqual(context.inductionTwoProjectValue, 10854725000);
  assert(context.inductionThreeReadyCardHtml.includes('Install Hybrid Turbo'));
  assert(context.inductionThreeReadyCardHtml.includes('Cost: $8B Cash'));
  assert(context.inductionThreeReadyCardHtml.includes('Build Value added: +$6B'));
  assert(context.inductionThreeReadyCardHtml.includes('factory-style housing'));
  assert.strictEqual(context.inductionThreePurchase.ok, true);
  assert.strictEqual(context.inductionThreeLevel, 3);
  assert.strictEqual(context.inductionThreeCashAfter, 1000000);
  assert.strictEqual(context.inductionThreeProjectValue, 16854725000);
  assert.strictEqual(context.inductionFourPurchase.ok, true);
  assert.strictEqual(context.inductionFourLevel, 4);
  assert.strictEqual(context.inductionFourCashAfter, 1000000);
  assert.strictEqual(context.inductionFourProjectValue, 25854725000);
  assert(context.inductionFiveReadyCardHtml.includes('Complete Induction Package'));
  assert(context.inductionFiveReadyCardHtml.includes('Cost: $18B Cash'));
  assert(context.inductionFiveReadyCardHtml.includes('Build Value added: +$14B'));
  assert(context.inductionFiveReadyCardHtml.includes('Anti-lag, water-meth'));
  assert.strictEqual(context.inductionFivePinned.title, 'Anti-Lag & Cooling Package');
  assert.strictEqual(context.inductionFivePurchase.ok, true);
  assert.strictEqual(context.inductionFivePurchase.feedback, 'Induction & Cooling complete: Anti-Lag & Cooling Package added. Garage Build Value +$14B.');
  assert.strictEqual(context.inductionFiveLevel, 5);
  assert.strictEqual(context.inductionFiveCashAfter, 1000000);
  assert.strictEqual(context.inductionFiveProjectValue, 39854725000);
  assert(context.inductionFiveNetWorth <= context.inductionFiveFormulaNetWorth);
  assert(context.inductionFiveNetWorth >= context.inductionFiveProjectValue);
  assert.strictEqual(context.inductionFiveProgress.completed, 28);
  assert.strictEqual(context.inductionFiveProgress.total, 40);
  assert(context.inductionFiveCardHtml.includes('Drivetrain &amp; Transmission'));
  assert(context.inductionFiveCardHtml.includes('Next Work: Sports Clutch &amp; Flywheel'));
  assert(context.inductionFiveCardHtml.includes('Cost: $27B Cash'));
  assert(context.inductionFiveCardHtml.includes('Build Value added: +$20B'));
  assert(context.inductionFivePanelHtml.includes('Induction &amp; Cooling'));
  assert(context.inductionFivePanelHtml.includes('Complete · Next Build Track: Drivetrain &amp; Transmission, future garage pass.'));
  assert(context.inductionFivePanelHtml.includes('Current focus: Drivetrain &amp; Transmission.'));
  assert(context.inductionFivePanelHtml.includes('Drivetrain &amp; Transmission'));
  assert(context.inductionFivePanelHtml.includes('Next Work: Sports Clutch &amp; Flywheel'));
  assert.strictEqual(context.inductionFivePinnedAfter.title, 'Sports Clutch & Flywheel');
  assert.strictEqual(context.inductionFiveTickPinned.title, 'Sports Clutch & Flywheel');
  assert.notStrictEqual(context.inductionFiveActionAfter.type, 'buy_dream_induction_work');
  assert.strictEqual(context.inductionFiveSecondPurchase.ok, false);

  assert(context.drivetrainOneReadyCardHtml.includes('Install Clutch &amp; Flywheel'));
  assert(context.drivetrainOneReadyCardHtml.includes('Cost: $27B Cash'));
  assert(context.drivetrainOneReadyCardHtml.includes('Build Value added: +$20B'));
  assert(context.drivetrainOneReadyCardHtml.includes('Sports flywheel and clutch-kit language'));
  assert.strictEqual(context.drivetrainOnePinned.title, 'Sports Clutch & Flywheel');
  assert.strictEqual(context.drivetrainOneAction.type, 'buy_dream_drivetrain_work');
  assert.strictEqual(context.drivetrainOnePurchase.ok, true);
  assert.strictEqual(context.drivetrainOneLevel, 1);
  assert.strictEqual(context.drivetrainOneCashAfter, 1000000);
  assert.strictEqual(context.drivetrainOneProjectValue, 59854725000);
  assert.strictEqual(context.drivetrainOneProgress.completed, 29);
  assert.strictEqual(context.drivetrainOneProgress.total, 40);
  assert.strictEqual(context.activeDrivetrainPurchase.ok, false);

  assert(context.drivetrainTwoReadyCardHtml.includes('Install LSD'));
  assert(context.drivetrainTwoReadyCardHtml.includes('Cost: $40B Cash'));
  assert(context.drivetrainTwoReadyCardHtml.includes('Build Value added: +$32B'));
  assert(context.drivetrainTwoReadyCardHtml.includes('1-way, 1.5-way, 2-way'));
  assert.strictEqual(context.drivetrainTwoPurchase.ok, true);
  assert.strictEqual(context.drivetrainTwoLevel, 2);
  assert.strictEqual(context.drivetrainTwoCashAfter, 1000000);
  assert.strictEqual(context.drivetrainTwoProjectValue, 91854725000);
  assert.strictEqual(context.drivetrainTwoProgress.completed, 30);

  assert(context.drivetrainThreeReadyCardHtml.includes('Upgrade Driveshaft'));
  assert(context.drivetrainThreeReadyCardHtml.includes('Cost: $60B Cash'));
  assert(context.drivetrainThreeReadyCardHtml.includes('Build Value added: +$50B'));
  assert(context.drivetrainThreeReadyCardHtml.includes('Carbon driveshaft'));
  assert.strictEqual(context.drivetrainThreePinned.title, 'Carbon Driveshaft & Axles');
  assert.strictEqual(context.drivetrainThreePurchase.ok, true);
  assert.strictEqual(context.drivetrainThreeLevel, 3);
  assert.strictEqual(context.drivetrainThreeCashAfter, 1000000);
  assert.strictEqual(context.drivetrainThreeProjectValue, 141854725000);
  assert.strictEqual(context.drivetrainThreeProgress.completed, 31);

  assert(context.drivetrainFourReadyCardHtml.includes('Build Custom Gearbox'));
  assert(context.drivetrainFourReadyCardHtml.includes('Cost: $90B Cash'));
  assert(context.drivetrainFourReadyCardHtml.includes('Build Value added: +$78B'));
  assert(context.drivetrainFourReadyCardHtml.includes('Custom manual transmission'));
  assert.strictEqual(context.drivetrainFourPinned.title, 'Custom Gearbox');
  assert.strictEqual(context.drivetrainFourPurchase.ok, true);
  assert.strictEqual(context.drivetrainFourLevel, 4);
  assert.strictEqual(context.drivetrainFourCashAfter, 1000000);
  assert.strictEqual(context.drivetrainFourProjectValue, 219854725000);
  assert.strictEqual(context.drivetrainFourProgress.completed, 32);

  assert(context.drivetrainFiveReadyCardHtml.includes('Complete Transmission'));
  assert(context.drivetrainFiveReadyCardHtml.includes('Cost: $135B Cash'));
  assert(context.drivetrainFiveReadyCardHtml.includes('Build Value added: +$120B'));
  assert(context.drivetrainFiveReadyCardHtml.includes('sequential transmission'));
  assert.strictEqual(context.drivetrainFivePinned.title, 'Sequential Transmission Package');
  assert.strictEqual(context.drivetrainFivePurchase.ok, true);
  assert.strictEqual(context.drivetrainFivePurchase.feedback, 'Drivetrain & Transmission complete: Sequential Transmission Package added. Garage Build Value +$120B.');
  assert.strictEqual(context.drivetrainFiveLevel, 5);
  assert.strictEqual(context.drivetrainFiveCashAfter, 1000000);
  assert.strictEqual(context.drivetrainFiveProjectValue, 339854725000);
  assert(context.drivetrainFiveNetWorth <= context.drivetrainFiveFormulaNetWorth);
  assert(context.drivetrainFiveNetWorth >= context.drivetrainFiveProjectValue);
  assert.strictEqual(context.drivetrainFiveProgress.completed, 33);
  assert.strictEqual(context.drivetrainFiveProgress.total, 40);
  assert(context.drivetrainFiveCardHtml.includes('Aero, Styling &amp; Weight Reduction'));
  assert(context.drivetrainFiveCardHtml.includes('Next Work: Front Splitter &amp; Side Skirts'));
  assert(context.drivetrainFiveCardHtml.includes('Cost: $200B Cash'));
  assert(context.drivetrainFiveCardHtml.includes('Build Value added: +$160B'));
  assert(context.drivetrainFivePanelHtml.includes('Drivetrain &amp; Transmission'));
  assert(context.drivetrainFivePanelHtml.includes('Complete · Next Build Track: Aero, Styling &amp; Weight Reduction.'));
  assert(context.drivetrainFivePanelHtml.includes('Current focus: Aero, Styling &amp; Weight Reduction.'));
  ...
  assert.strictEqual(context.drivetrainFiveSecondPurchase.ok, false);

  assert(context.aeroOneReadyCardHtml.includes('Fit Splitter &amp; Skirts'));
  assert(context.aeroOneReadyCardHtml.includes('Cost: $200B Cash'));
  assert(context.aeroOneReadyCardHtml.includes('Build Value added: +$160B'));
  assert(context.aeroOneReadyCardHtml.includes('Front splitter / lip spoiler'));
  assert.strictEqual(context.aeroOnePinned.title, 'Front Splitter & Side Skirts');
  assert.strictEqual(context.aeroOneAction.type, 'buy_dream_aero_work');
  assert.strictEqual(context.aeroOnePurchase.ok, true);
  assert.strictEqual(context.aeroOneLevel, 1);
  assert.strictEqual(context.aeroOneCashAfter, 1000000);
  assert.strictEqual(context.aeroOneProjectValue, 499854725000);
  assert.strictEqual(context.aeroOneProgress.completed, 34);
  assert.strictEqual(context.aeroOneProgress.total, 40);
  assert.strictEqual(context.activeAeroPurchase.ok, false);

  assert(context.aeroTwoReadyCardHtml.includes('Fit Rear Aero'));
  assert(context.aeroTwoReadyCardHtml.includes('Cost: $300B Cash'));
  assert(context.aeroTwoReadyCardHtml.includes('Build Value added: +$240B'));
  assert(context.aeroTwoReadyCardHtml.includes('swan-neck wing mount'));
  assert.strictEqual(context.aeroTwoPurchase.ok, true);
  assert.strictEqual(context.aeroTwoLevel, 2);
  assert.strictEqual(context.aeroTwoCashAfter, 1000000);
  assert.strictEqual(context.aeroTwoProjectValue, 739854725000);
  assert.strictEqual(context.aeroTwoProgress.completed, 35);

  assert(context.aeroThreeReadyCardHtml.includes('Fit Wide Body'));
  assert(context.aeroThreeReadyCardHtml.includes('Cost: $450B Cash'));
  assert(context.aeroThreeReadyCardHtml.includes('Build Value added: +$360B'));
  assert(context.aeroThreeReadyCardHtml.includes('Wide body modification'));
  assert.strictEqual(context.aeroThreePinned.title, 'Wide Body & Vented Panels');
  assert.strictEqual(context.aeroThreePurchase.ok, true);
  assert.strictEqual(context.aeroThreeLevel, 3);
  assert.strictEqual(context.aeroThreeCashAfter, 1000000);
  assert.strictEqual(context.aeroThreeProjectValue, 1099854725000);
  assert.strictEqual(context.aeroThreeProgress.completed, 36);
  assert(context.aeroOverviewSummary.includes('Aero: Wide Body &amp; Vented Panels'));
  assert(context.aeroOverviewSummary.includes('36 / 40'));
  assert(!context.aeroOverviewSummary.includes('louvered fender'));

  assert(context.aeroFourReadyCardHtml.includes('Reduce Weight'));
  assert(context.aeroFourReadyCardHtml.includes('Cost: $675B Cash'));
  assert(context.aeroFourReadyCardHtml.includes('Build Value added: +$540B'));
  assert(context.aeroFourReadyCardHtml.includes('Lexan / polycarbonate windows'));
  assert.strictEqual(context.aeroFourPinned.title, 'Weight Reduction Package');
  assert.strictEqual(context.aeroFourPurchase.ok, true);
  assert.strictEqual(context.aeroFourLevel, 4);
  assert.strictEqual(context.aeroFourCashAfter, 1000000);
  assert.strictEqual(context.aeroFourProjectValue, 1639854725000);
  assert.strictEqual(context.aeroFourProgress.completed, 37);

  assert(context.aeroFiveReadyCardHtml.includes('Complete Aero Package'));
  assert(context.aeroFiveReadyCardHtml.includes('Cost: $1T Cash'));
  assert(context.aeroFiveReadyCardHtml.includes('Build Value added: +$800B'));
  assert(context.aeroFiveReadyCardHtml.includes('roll cage Type A'));
  assert.strictEqual(context.aeroFivePinned.title, 'Carbon Body & Roll Cage');
  ...
  assert(context.dreamOverviewSummary.includes('Suspension: Showcase Stance'));
  assert(context.dreamOverviewSummary.includes('13 / 40'));
  assert(!context.dreamOverviewSummary.includes('fully customizable coilovers'));
  assert(!context.dreamOverviewSummary.includes('camber/toe arms'));
  ...
  assert(context.dreamSummaryCard.includes('13 / 40 work stages'));
...
const lowCashComplete = normalizeGameState(complete);
lowCashComplete.shop.tips = Math.max(0, showcaseEconomics.entryCost - 1);
globalThis.lowCashPanel = renderCarManagementPanel(lowCashComplete);
...
const showcaseStartRenderState = normalizeGameState(showcaseStart.gameState);
showcaseStartRenderState.carManagement.activeAssignment.startedAt = new Date(Date.now()).toISOString();
globalThis.showcaseStartPanel = renderCarManagementPanel(showcaseStartRenderState);
...
const halfwayRenderState = normalizeGameState(showcaseStart.gameState);
halfwayRenderState.carManagement.activeAssignment.startedAt = new Date(Date.now() - 7 * 60 * 1000).toISOString();
globalThis.halfwayPanel = renderCarManagementPanel(halfwayRenderState);
...
const readyRenderState = normalizeGameState(showcaseStart.gameState);
readyRenderState.carManagement.activeAssignment.startedAt = new Date(Date.now() - 16 * 60 * 1000).toISOString();
globalThis.readyPanel = renderCarManagementPanel(readyRenderState);
...
globalThis.showcaseCollectedPanel = renderCarManagementPanel(showcaseCollectedState);
...
globalThis.longHistoryPanel = renderCarManagementPanel(longHistoryState);
...
`, context);
  ...
  assert(context.carManagementPanel.includes('Managed Car'));
  assert(context.carManagementPanel.includes('First Complete Build'));
  assert(context.carManagementPanel.includes('Core Build Complete · 40 / 40'));
  assert(context.carManagementPanel.includes('Garage Build Value at completion'));
  assert(context.carManagementPanel.includes('Builder Note: &quot;Built after too many tofu shifts.&quot;'));
  assert(context.carManagementPanel.includes('Status: Available'));
  ...
  assert(context.carManagementPanel.includes('State: Available'));
  assert(context.carManagementPanel.includes('State: Locked'));
  assert(context.carManagementPanel.includes('Cost: '));
  assert(context.carManagementPanel.includes('Duration: 15m'));
  assert(context.carManagementPanel.includes('Rewards: +'));
  assert(context.carManagementPanel.includes('Net Cash: +'));
  assert(context.carManagementPanel.includes('Complete Showcase Rotation once and reach 25 Garage Reputation.'));
  assert(context.carManagementPanel.includes('First Car Management Loop'));
  assert(context.carManagementPanel.includes('Garage Event Board is the one-time event progression board'));
  assert(context.carManagementPanel.includes('does not affect'));
  assert(context.lowCashPanel.includes('Need $1 more Cash.'));
  assert(context.lowCashPanel.includes('disabled'));
  ...
  assert(context.showcaseStartPanel.includes('Status: On assignment'));
  assert(context.showcaseStartPanel.includes('Current Assignment: Showcase Rotation'));
  assert(context.halfwayPanel.includes('State: Active'));
  assert(context.halfwayPanel.includes('Returns in'));
  assert(context.halfwayPanel.includes('% complete'));
  assert(context.halfwayPanel.includes('One assignment at a time. The car is currently at Showcase Rotation.'));
  assert(!context.halfwayPanel.includes('data-car-assignment-start="sponsor_demo_day"'));
  ...
  assert(context.readyPanel.includes('Status: Ready to collect'));
  assert(context.readyPanel.includes('Ready to collect'));
  assert(context.readyPanel.includes('Rewards waiting: +'));
  assert(context.readyPanel.includes('Collect Rewards'));
  ...
  assert(context.showcaseCollectedPanel.includes('Completed 1 time'));
  assert(context.showcaseCollectedPanel.includes('Available again'));
  assert(context.showcaseCollectedPanel.includes('Recent Assignment Results'));
  assert(context.showcaseCollectedPanel.includes('Showcase Rotation · +'));
...
`, context);
  ...
  assert(!context.orderTypePanelHtml.includes('More status'));
...
`, context);
  ...
  assert(!context.nextMilestoneFreshHtml.includes('More status'));
...
renderTofuShop(${JSON.stringify(fresh)});
  ...
  assert(context.counterFreshHtml.includes('Income:'));
...
globalThis.counterUnlockedAction = nextBestAction(${JSON.stringify(unlocked)}, { date: new Date('2026-06-15T12:00:00.000Z') });
  ...
  assert(!context.counterUnlockedHtml.includes('More status'));
...
const maxSpirit = ${JSON.stringify(spirit)};
maxSpirit.shop.tips = 1e9;
for (const generator of SPIRIT_GENERATORS) maxSpirit.shop.spiritGenerators[generator.id] = 25;
renderTofuShop(maxSpirit);
globalThis.maxSpiritPanelHtml = elements.shopTabPanel.innerHTML;
globalThis.maxSpiritBulk = buyAllAffordableSpiritGenerators(maxSpirit, { activeDrive: false });
const nearMaxSpirit = ${JSON.stringify(spirit)};
nearMaxSpirit.shop.tips = 1e9;
for (const generator of SPIRIT_GENERATORS) nearMaxSpirit.shop.spiritGenerators[generator.id] = 25;
nearMaxSpirit.shop.spiritGenerators.tea_kettle = 24;
globalThis.nearMaxSpiritBulk = buyAllAffordableSpiritGenerators(nearMaxSpirit, { activeDrive: false });
`, context);
  ...
  assert(context.maxSpiritPanelHtml.includes('Tea Kettle Lv 25 / 25'));
  assert(context.maxSpiritPanelHtml.includes('Lucky Cat Lv 25 / 25'));
  assert(context.maxSpiritPanelHtml.includes('Maxed'));
  assert.strictEqual(context.maxSpiritBulk.ok, false);
  assert(context.maxSpiritBulk.reason.includes('maxed'));
  assert.strictEqual(context.nearMaxSpiritBulk.ok, true);
  assert.strictEqual(context.nearMaxSpiritBulk.bought, 1);
  assert.strictEqual(context.nearMaxSpiritBulk.gameState.shop.spiritGenerators.tea_kettle, 25);
  assert(!context.spiritPanelHtml.includes('Use Boost'));
  assert(!context.spiritPanelHtml.includes('Use Festival Boost'));
  assert(context.spiritPanelHtml.includes('Activate · 10 Spirit'));
...
`, context);
  ...
  assert(rushResult.feedback.includes('Rush Stock activated: +'));
  ...
  assert(warmResult.feedback.includes('Warm Counter activated: +'));
...
`, context);
  ...
  assert(html.includes('/static/nospill/app.js?v=20260623c'));
  assert(html.includes('/static/nospill/app.css?v=20260623c'));
}
...
appState.surface = "shop";
appState.shopTab = "overview";
saveGameState(offlineLong);
renderTofuShop(offlineLong);
globalThis.generousOfflineSummaryFirst = offlineElements.shopOfflineEarnings.textContent;
globalThis.generousOfflineSummaryHtml = offlineElements.shopOfflineEarnings.innerHTML;
const ledgerClickTarget = { dataset: { shopTab: "ledger" }, closest() { return this; } };
handleTofuShopPanelClick({ target: ledgerClickTarget });
globalThis.offlineLedgerClickTab = appState.shopTab;
appState.shopTab = "overview";
renderTofuShop(offlineLong);
...
`, context);
  ...
  assert(context.generousOfflineSummaryHtml.includes('data-shop-tab="ledger"'));
  assert(context.generousOfflineSummaryHtml.includes('>Ledger</button>'));
  assert.strictEqual(context.offlineLedgerClickTab, 'ledger');
  ...
  const suggestions = context.generousOfflineSummaryFirst.split('Next: ')[1].split(' · Ledger')[0].split(' · ');
...
globalThis.queueFullOverviewHtml = renderOverviewPanel(queueFull);
...
appState.detailOpenState = {};
globalThis.detailsClosedHtml = compactDetails("overview_order_details", "How orders work", "<p>Order details.</p>");
handleShopDetailsToggle({ target: { matches: (selector) => selector === "details[data-details-key]", dataset: { detailsKey: "overview_order_details" }, open: true } });
globalThis.detailsOpenHtml = compactDetails("overview_order_details", "How orders work", "<p>Order details.</p>");
handleShopDetailsToggle({ target: { matches: (selector) => selector === "details[data-details-key]", dataset: { detailsKey: "overview_order_details" }, open: false } });
globalThis.detailsReclosedHtml = compactDetails("overview_order_details", "How orders work", "<p>Order details.</p>");
...
`, context);
  ...
  assert(context.queueFullOverviewHtml.includes('Tofu Shop'));
  assert(context.queueFullOverviewHtml.includes('Queue full · Counter Service running'));
  assert(context.queueFullOverviewHtml.includes('Ready Orders:'));
  assert(context.queueFullOverviewHtml.includes('Income:'));
  assert(context.queueFullOverviewHtml.includes('Prep Capacity:'));
  assert(context.queueFullOverviewHtml.includes('How orders work'));
  assert(context.queueFullOverviewHtml.includes('Preparing Next Order'));
  assert(context.queueFullOverviewHtml.includes('Optional Certified Boost'));
  assert(!context.queueFullOverviewHtml.includes('More status'));
  assert(!context.queueFullOverviewHtml.includes('Visible'));
  assert(!context.queueFullOverviewHtml.includes('data-details-key="overview_more_status"'));
  assert(context.queueFullOverviewHtml.indexOf('Tofu Shop') < context.queueFullOverviewHtml.indexOf('Preparing Next Order'));
  assert(context.queueFullOverviewHtml.includes('data-details-key="overview_order_details"'));
  assert(context.queueFullOverviewHtml.includes('Prepared orders fill the Delivery Orders queue.'));
  assert(context.queueFullOverviewHtml.includes('Prep Capacity is the recovering expansion pool'));
  assert(!context.queueFullOverviewHtml.includes('Prep Capacity can be recharged'));
  assert(!context.queueFullOverviewHtml.includes('Upgrade Prep Capacity'));

  assert.strictEqual(context.capPinned.id, 'suspension-refreshed');
  assert(context.capGoalHtml.includes('Suspension Refreshed'));
  ...
  assert(!context.detailsClosedHtml.includes(' open>'));
  assert(context.detailsOpenHtml.includes(' open>'));
  assert(!context.detailsReclosedHtml.includes(' open>'));
...
function testSuperCuteCollectiblesLandingAndMerchCopy() {
  const html = fs.readFileSync(NOSPILL_HTML, 'utf8');
  assert(html.includes('Secret Merch'));
  assert(html.includes('Hidden stickers and shirts reveal after local milestones.'));
  assert(html.includes('Product links stay hidden until unlocked.'));
