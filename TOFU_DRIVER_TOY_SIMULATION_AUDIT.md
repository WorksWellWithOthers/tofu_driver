...
| Toy-first design | Cup Trail, Tofu Garage living scene, Passport stamps, Dream Build progress, Garage Event Board V1, local Builder Notes, Delivery Crew cosmetic surface, polished story-card previews, share cards, Result Story captions, Cargo Commentary, fanfares. | Partial | Most toys are still display/status objects; the event board adds a first one-time parked outcome loop, but captioning, generated commentary, story-card previews, and Builder Notes are still small story tools, not full open-ended creation. | Playtest local expression tools before adding larger editors. | P1 |
| Open-ended play | Tofu Garage can grow through shop scaling, Dream Build, Garage Event Board V1, Net Worth, Brand Value, Garage Reputation, stamps, and cosmetics. | Partial | Current runtime still mostly guides the player through predetermined upgrade paths and one-time event choices. | Add local story/expression tools before deeper economy branches. | P2 |
| Creative leverage/customization | Delivery Crew selection, sound/character collections, Cup Trail result flavor, Dream Build identity, local Builder Notes, share-card output, local result captions, and the standalone Tofu Garage tuning catalog. | Partial | Build choices remain mostly linear; the catalog is source material and category preview, not a full customization system yet; shop/garage decoration and multi-panel comic tools are not implemented. | Playtest Builder Note usefulness and the first tuning tracks, then scope decoration only if needed. | P1 |
...
captions, Cargo Commentary, and cosmetics give the player toy-like objects, but creative leverage is
still narrow. The current game now lets the player author one safe caption on a result card, adds
deterministic generated cargo flavor, keeps advanced run accounting collapsed by default, and lets
